...
class KeywordCaptionizer extends React.Component {
  ...
  addKeyword(keyword) {
    ...
    if (this.validateKeyword(keyword)) {
      this.setState(
        ...
        () => {
          document
            .querySelector(".kw-div:last-child")
  ...
  removeKeyword(index) {
    document
      .querySelector(`.kw-div:nth-child(${index + 1})`)
  ...
  validateKeyword(keyword) {
    ...
    if (this.state.keywords.length === 20) {
      ...
      this.state.keywords.find((val) => val === keyword) !== undefined
    ) {
  ...
  getQuotes() {
    ...
    this.state.keywords?.map((elem) =>
      fetch(`https://api.quotable.io/search/quotes?query=${elem}`, {
        method: "GET",
      })
        .then((res) => res.json())
        .then((data) => {
    ...
    return (
      <div className="kc-div">
          ...
          <div className="kw-input-div">
            <div className="input-div" id="input-div">
              <input
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter")
                    this.addKeyword(document.getElementById("kw-input").value);
                  this.handleKeydown();
                }}
        ...
        <div className="upload-div">
          <button onClick={this.clearAll} disabled={disabled}>
            Clear All
          </button>
          <button onClick={this.getQuotes} disabled={disabled}>
            Get Quotes
          </button>
        </div>
        {this.state.error ? <FetchError retry={this.retry}></FetchError> : null}
        {this.state.quotes ? (
          <QuoteDisplay
